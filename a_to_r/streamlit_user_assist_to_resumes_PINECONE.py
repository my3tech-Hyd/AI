# streamlit_user_assist_to_resumes_PINECONE.py
# Assistance Posting → Resume Search (Pinecone + BM25 Hybrid)
# ----------------------------------------------------------------------
...
# ========================= Page Config =========================
st.set_page_config(
    page_title="Assistance → Resume Search (Pinecone)",
    page_icon="🧭",
    layout="wide"
)
st.title("🧭 Assistance Posting → Resume Search (Pinecone Hybrid)")
st.caption("Enter assistance center details and find the best matching candidate resumes using hybrid retrieval (Pinecone + BM25)")
...
with st.form("assistance_form_ui", clear_on_submit=False):
    st.subheader("📝 Enter Assistance Center Details")
    
    center_name = st.text_input("Assistance Center Name", value="CareerPath Assistance Center")
    services = st.text_input("Services Offered", value="Career Counseling, Resume Writing, Interview Preparation")
    operating_hours = st.text_input("Operating Hours", value="Mon–Fri 9:00 AM – 6:00 PM")
    
    description = st.text_area(
        "Assistance Center Description",
        value="CareerPath Assistance Center helps job seekers achieve their career goals through personalized guidance, expert resume building, intensive interview preparation, and ongoing job placement support. We specialize in IT sector placements.",
        height=200,
    )
...
if run_btn:
    if not center_name or not services:
        ...
        # Compose assistance query
        query_parts = []
        if center_name:
            query_parts.append(f"Assistance Center: {center_name}")
        if services:
            query_parts.append(f"Services: {services}")
        if operating_hours:
            query_parts.append(f"Operating Hours: {operating_hours}")
        if description:
            ...
            try:
                ...
                if not results:
                    ...
                    st.download_button(
                        ...
                        data=csv,
                        file_name="assistance_to_resumes_results.csv",
                        mime="text/csv",
                    )
...
st.markdown("---")
st.caption("💡 **Tip:** This tool searches the 'resumes' namespace to find candidates who could benefit from your assistance center services.")
