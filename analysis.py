...
# ----------------------------------------------------------------------
# Resume â†”ï¸Ž Training Center Analyst (GPT-4o-mini)
# - User uploads ONE resume file
# - User fills the Training Center / Skill Posting form (provided fields)
# - We convert resume â†’ JSON (light heuristics) and collect training form â†’ JSON
...
# -------------- App config --------------
st.set_page_config(
    page_title="Resume â†”ï¸Ž Training Center Analyst (GPT-4o-mini)",
    page_icon="ðŸŽ¯",
    layout="wide",
)
st.title("ðŸŽ¯ Resume â†”ï¸Ž Skill/Training Analysis (GPT-4o-mini)")
...
def get_openai_client() -> OpenAI:
    ...
    """
    api_key = os.getenv("OPENAI_API_KEY", "sk-<REDACTED>") or st.secrets.get("OPENAI_API_KEY", "")
    if not api_key:
        with st.sidebar:
...
def read_text_from_bytes(data: bytes, filename: str) -> str:
    ...
    try:
        if ext == ".pdf":
            ...
            # .doc or unknown â†’ best-effort decode
            text = data.decode(errors="ignore")
    except Exception:
        text = data.decode(errors="ignore")
...
1) Executive Summary (6â€“10 sentences): clear rationale for pursuing this training now.
2) Strengths (8â€“12 bullets): specific capabilities from the resume that will accelerate learning in these courses.
3) Skill Gaps & Learning Needs (8â€“12 bullets): gaps mapped to precise modules or topics from the offered courses.
4) Course Fit & Roadmap:
   - Rank top 2â€“3 courses from the center by impact for this candidate.
   - For each: what theyâ€™ll learn, prerequisites assumed, how it fixes gaps.
   - Timeline aligned to â€œCourse Durationâ€ with weekly milestones.
5) Project & Portfolio Plan:
   - 3â€“5 project ideas tied to the courses (with crisp acceptance criteria, datasets/tech suggestions).
   - GitHub/portfolio structure and documentation notes.
6) Certifications & Assessments:
   - How the centerâ€™s certifications (and any third-party ones) add credibility; mapping to roles.
7) Interview & Career Outcomes:
   - Expected interview topics theyâ€™ll be able to answer after training.
   - Resume bullet rewrites (4â€“6 lines) post-training outcome.
8) Risk & Mitigation:
...
with left:
    st.subheader("ðŸ« Training Center / Skill Posting")
...
if analyze_btn:
    ...
    with st.expander("ðŸ‘€ View Parsed Inputs (JSON)"):
    ...
    # Render output
    st.markdown("---")
    st.subheader("ðŸ“Š Training Fit & Improvement Analysis")
    st.markdown(content)
    ...
    st.download_button(
        "â¬‡ï¸ Download Analysis (Markdown)",
