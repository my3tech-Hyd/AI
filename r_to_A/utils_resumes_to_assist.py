# utils_resumes_to_assist.py
# ----------------------------------------------------------------------
# Utilities for: Resume → Assistance Center Postings retrieval (cosine-only)
...
# - Helper to open the ASSISTANCE POSTS Chroma collection (admin-ingested)
# ----------------------------------------------------------------------
...
def _chunk_query_text(
    text: str,
    size: int = R2A_QUERY_CHUNK_SIZE,
    overlap: int = R2A_QUERY_CHUNK_OVERLAP,
    max_chunks: int = R2A_QUERY_MAX_CHUNKS,
    max_chars: int = R2A_MAX_QUERY_CHARS,
) -> List[str]:
    """
...
def get_assist_posts_collection():
    """
    Open (or create if missing) the dedicated ASSISTANCE postings collection.
    ...
    return client.get_or_create_collection(name=CHROMA_COLLECTION_ASSIST_POSTS)
